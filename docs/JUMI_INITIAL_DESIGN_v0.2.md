...
> 수정 메모: data-plane app 경계, external queue/pipeline 경계, in-cluster gRPC mesh 방향 반영
...
- JUMI는 Kubernetes API, Kueue, 그리고 같은 클러스터 내부의 다른 app과 통신한다.
...
JUMI is a policy-agnostic in-cluster execution interface for managing run, node, and attempt units on Kubernetes.

한국어로는 다음과 같이 정리할 수 있다.

JUMI는 Kubernetes 클러스터 내부에서 run, node, attempt 단위를 실행·제어·관찰하는 policy-agnostic execution data-plane interface다.
