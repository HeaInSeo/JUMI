...
> 수정 메모: data-plane app 경계, external queue/pipeline 경계, in-cluster gRPC mesh 방향, optional Kueue integration 반영
...
- Redis Streams queue와 pipeline lowering을 JUMI 바깥 계층으로 고정한다.
- JUMI를 Kubernetes 내부 data-plane app으로 위치시키고, 다른 app과의 통신 원칙을 명시한다.
- Kueue를 optional integration으로 두고, Kueue가 없어도 JUMI가 정상 동작해야 한다는 기준을 고정한다.
...
JUMI는 executable run spec을 받아 Kubernetes에서 실행/관찰/제어하는 execution core이자 data-plane app이다.
...
- 이벤트 및 병목 원인 기록
- 클러스터 내부 다른 app과의 gRPC 기반 제어/상태 통신
- Kubernetes native execution backend와의 연동
- Kueue가 있을 경우 이를 선택적으로 활용하는 integration
...
- Redis Streams 기반 durable ingress queue 구현
- tenant fairness / retry class / backoff class 정책
- 장기 durable scheduler 기능
- 특정 queue system 의존적인 실행 의미론
...
- Data Plane App: control-plane 판단 결과를 받아 실제 실행/제어/관찰을 수행하는 클러스터 내부 애플리케이션
- Optional Kueue Integration: Kueue가 존재하면 활용하지만, 없어도 core execution semantics가 유지되는 연동 방식
...
- JUMI는 policy scheduler 자체도 아니다.
- JUMI는 실행 요청을 실제 Kubernetes backend 동작으로 바꾸는 in-cluster execution service다.

초기 배치 가정:

- JUMI는 Kubernetes Deployment 또는 이와 동등한 방식으로 클러스터 내부에 상주시킨다.
- JUMI는 Kubernetes API, 선택적으로 Kueue, 그리고 같은 클러스터 내부의 다른 app과 통신한다.
- JUMI의 northbound 호출자는 외부 scheduler일 수도 있고, 클러스터 내부 companion service일 수도 있다.

### 2.2 서비스 간 통신

JUMI는 다른 app들과의 내부 통신에 gRPC를 사용한다.

초기 원칙:

- northbound/southbound service-to-service 통신은 기본적으로 gRPC로 둔다.
- 내부 service discovery 및 네트워크 경로는 Cilium mesh 방식을 기준선으로 둔다.
- HTTP는 health/readiness/status 같은 운영 endpoint에 우선 사용하고, app 간 제어 인터페이스는 gRPC를 우선한다.
- 메시 네트워크가 제공하는 identity, routing, observability를 활용하되, JUMI 비즈니스 의미론은 mesh 구현에 종속되지 않도록 유지한다.

### 2.3 통신 경계 원칙

- pipeline lowering 결과 전달은 gRPC 또는 동등한 RPC 계약으로 받을 수 있다.
- policy scheduler가 admission 판단을 마친 executable run spec만 JUMI로 들어온다.
- Redis Streams는 JUMI 내부 transport가 아니라 외부 queue/scheduler 계층의 구현 선택지다.
- JUMI는 queue를 polling하는 app이 아니라, 실행 명세를 받아 수행하는 app으로 남는다.
...
In-cluster apps
  ...
  -> gRPC
  -> JUMI
  -> Kubernetes API / optional Kueue / backend
```
...
- Kueue는 optional integration이며, JUMI core는 Kueue 부재 시에도 정상 동작해야 한다.
...
외부 또는 클러스터 내부 app으로부터 실행 요청을 받는 계층이다.
...
Kueue는 이 backend 위에 선택적으로 붙는 integration이다.
향후 다른 backend가 생기더라도 JUMI core 변경은 최소화해야 한다.

원칙:

- Kueue가 없다고 해서 prepare/start가 실패해서는 안 된다.
- Kueue가 있으면 queue label, suspended job, workload 관찰 같은 기능을 사용할 수 있다.
- Kueue 관련 필드가 없더라도 Kubernetes native 실행은 가능해야 한다.

### 4.5 Bounded Release Controller
...
- Kueue가 없는 배포에서는 `kueue_pending`이 없어도 정상이어야 한다.
...
- Kueue 전용 입력이 없어도 spec는 유효할 수 있어야 한다.
...
- Kueue 관련 메타데이터는 optional이어야 한다.
...
### 7.2 backend semantics

JUMI의 기본 backend semantics는 Kubernetes native execution이다.

초기 규칙:

- Kueue가 없어도 Kubernetes Job 기반 실행은 정상 동작해야 한다.
- Kueue 존재 여부는 실행 가능성의 전제조건이 아니다.
- Kueue integration은 queue admission과 관찰 강화를 위한 optional feature다.
- Kueue 부재를 이유로 run 제출이나 backend start가 실패해서는 안 된다.

### 7.3 bounded release semantics
...
JUMI는 두 종류의 endpoint를 가진다.

- gRPC: app-to-app 제어 및 상태 조회 인터페이스
- HTTP: health / readiness / status 같은 운영 인터페이스

초기 제품 기준에서는 app 간 계약을 gRPC 우선으로 둔다.

### 8.2 Run 제출 API

`SubmitRun` 또는 이에 대응하는 `POST /runs`
...
`ListRunNodes` 또는 `GET /runs/{runId}/nodes`
...
`CancelRun` 또는 `POST /runs/{runId}:cancel`
...
- mesh 및 gRPC listener 설정은 deployment/config 수준에서 관리한다.
- Kueue 사용 여부는 deployment/config 수준의 optional capability로 다룬다.
...
- metrics enablement
- gRPC listen address
- mesh identity / policy 연동 설정
- kueue integration enabled
...
- in-cluster gRPC 통신 오류가 stop cause 또는 운영 지표로 드러나는가
- Kueue 부재 시에도 run이 정상 실행되는가
...
- Kueue는 기준선 backend가 아니라 optional layer로 본다.
...
- JUMI는 Redis Streams consumer group을 직접 운영하지 않는다.
- JUMI는 Redis pending/ack/reclaim 의미론을 내부 상태 모델의 기준으로 삼지 않는다.
- Redis Streams는 JUMI로 실행 명세를 전달하는 외부 transport로만 연결될 수 있다.

### 16.2 Pipeline

pipeline authored model과 lowering은 아직 준비되지 않은 외부 계층이다.
JUMI는 이 계층의 미완성과 분리되어 독립적으로 구현 가능해야 한다.

원칙:

- JUMI는 authored pipeline schema에 의존하지 않는다.
- JUMI는 executable run spec만 계약 대상으로 본다.
- pipeline lowering이 완성되기 전까지 fixture 기반 executable spec으로 JUMI를 구현/검증한다.

### 16.3 Kueue

Kueue는 JUMI의 필수 전제조건이 아니다.
JUMI는 Kueue가 없어도 정상 실행되어야 한다.

원칙:

- JUMI core execution은 Kubernetes native backend 위에서 성립해야 한다.
- Kueue는 admission/queue 관찰을 강화하는 선택적 integration이다.
- Kueue 관련 기능이 비활성화되거나 설치되지 않아도 run submit/start/wait/cancel은 정상이어야 한다.
- Kueue 관련 오류는 optional integration failure로 다뤄야 하며, core backend failure와 구분해야 한다.
...
3. dag-go wiring
4. Kubernetes native backend adapter 연결
...
- JUMI는 Kubernetes 내부에 배치되는 data-plane app이다.
- JUMI는 executable run spec 기반 execution app이다.
- DAG readiness 판단은 dag-go를 기준으로 한다.
- backend execution은 Kubernetes native path를 기본으로 하고, Kueue는 optional integration으로 둔다.
- bounded release는 scheduler가 아니라 pacing controller다.
- Redis Streams queue는 외부 계층에 남긴다.
- pipeline lowering은 외부 계층에 남긴다.
- app 간 제어 인터페이스는 gRPC를 우선한다.
- 내부 네트워크 기준선은 Cilium mesh 방식이다.
...
JUMI is a policy-agnostic in-cluster execution interface for managing run, node, and attempt units on Kubernetes, with optional Kueue integration.

한국어로는 다음과 같이 정리할 수 있다.

JUMI는 Kubernetes 클러스터 내부에서 run, node, attempt 단위를 실행·제어·관찰하는 policy-agnostic execution data-plane interface이며, Kueue는 선택적으로 연동된다.
